...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        mergePolygons = action(
            self.tr("Merge Polygons"),
            self.mergeSelectedShapes,
            shortcuts.get("merge_polygons"),
            icon=None,
            tip=self.tr("Merge selected polygons into one"),
            enabled=False,
        )
        paste = action(
        ...
        addMultiplePoints = action(
            text=self.tr("Add Multiple Points"),
            slot=self.addMultiplePointsToEdge,
            shortcut=shortcuts.get("add_multiple_points"),
            icon=None,
            tip=self.tr("Add multiple points to selected edge"),
            enabled=False,
        )
...
""",
            ...
            addMultiplePoints=addMultiplePoints,
        ...
        self.canvas.edgeSelected.connect(self.actions.addMultiplePoints.setEnabled)
    ...
    def shapeSelectionChanged(self, selected_shapes):
        ...
        self.actions.mergePolygons.setEnabled(n_selected >= 2)
    ...
    def mergeSelectedShapes(self):
        ...
        if merged_shapes:
            # Reload the label list with updated shapes
            self.labelList.clear()
            ...
            if len(merged_shapes) == 1:
                ...
                QMessageBox.information(
                    ...
                    self.tr(f"Merged {original_count} polygons into 1 polygon"),
                )
    ...
    def addMultiplePointsToEdge(self):
        """Prompt user for number of points and add them to the selected edge"""
        num_points, ok = QtWidgets.QInputDialog.getInt(
            self,
            self.tr("Add Multiple Points"),
            self.tr("Number of points to add:"),
            value=self._config.get("default_num_points_to_add", 1),
            min=1,
            max=100,
        )
        if ok:
            self.canvas.addMultiplePointsToEdge(num_points)
            self.canvas.update()
            self.setDirty()
            if self.noShapes():
                for action in self.on_shapes_present_actions:
