...
class Canvas(QtWidgets.QWidget):
    ...
    def mouseMoveEvent(self, a0: QtGui.QMouseEvent) -> None:
        ...
        for shape in ([self.hShape] if self.hShape else []) + [
            ...
            if index_edge is not None and shape.canAddPoint():
                ...
                status_messages.append(self.tr("Ctrl+M to add multiple points"))
                self.update()
                break
    ...
    def addMultiplePointsToEdge(self, num_points: int = 1):
        """Add multiple evenly-spaced points along the highlighted edge"""
        shape = self._lasthShape
        index = self._lasthEdge
        if shape is None or index is None:
            return
        shape.addMultiplePointsToEdge(index, num_points)
